...
sudo apt update
sudo apt install bc gawk curl ffmpeg git build-essential autoconf automake libtool pkg-config python3-dev cython3 libffms2-4 libarchive-tools $DesktopPkg
mkdir Gits; cd Gits
...
# Vapoursynth install
wget https://github.com/vapoursynth/vapoursynth/archive/refs/tags/R54.zip
unzip R54.zip
cd vapoursynth-R54
./autogen.sh && ./configure && make -j$(nproc)
sudo make install; cd ..
...
if [ ! -z $DISPLAY ]; then
	git clone https://github.com/YomikoR/vapoursynth-editor.git; cd vapoursynth-editor/pro
