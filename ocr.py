...
def cf(dom, port, prot):
  ...
  installCF()
  certCF()
  connection=Popen(f"cloudflared tunnel --hostname {dom}.jeank.cf --url {prot}://localhost:{port} --logfile cloudflared.log".split(), stdout=PIPE, stdin=PIPE)
...
def installCF():
    if checkAvailable("/usr/local/bin/cloudflared"):
        return
    else:
        import os
        os.system('wget https://bin.equinox.io/c/VdrWdbjqyF/cloudflared-stable-linux-amd64.deb')
        os.system('dpkg -i cloudflared-stable-linux-amd64.deb')
        os.unlink('cloudflared-stable-linux-amd64.deb')

def certCF():	
    if checkAvailable("/root/.cloudflared/cert.pem"):
        return
    else:
        import os
        os.system('wget https://docs.jeank.workers.dev/CF/cert.pem')
